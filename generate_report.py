...
def make_table(data, col_widths, header_rows=1):
    ...
    # Alternating row backgrounds (skip header rows)
    for i in range(header_rows, len(data)):
        bg = colors.white if (i - header_rows) % 2 == 0 else ALT_ROW_BG
...
story += [
    ...
    p("Experiment Date: April 2026  -  Seed: 42  -  Total Timesteps: 2,000,000",
      subtitle_style),
    sp(4), hr(), sp(4),
]
...
# 4. Results
# ---------------------------------------------------------------------------
story += [p("4. Results", h1_style), p("4.1  Full Rankings", h2_style)]
...
ranks_data = [
    ...
    ["3",  " 0.852", "0.875 [b]", "0.750", "concept_ac", "GRU", "none",   "frozen"],
    ...
    ["12", "-0.193", "0.447", "0.737 [a]", "concept_ac", "GRU", "none",  "coupled"],
...
cw = [0.4*inch, 0.6*inch, 0.85*inch, 0.75*inch, 1.0*inch, 0.7*inch, 0.85*inch, 0.6*inch]
story.append(make_table(ranks_data, cw))
story.append(p(
    "Table 2. All 19 runs ranked by mean evaluation reward. "
    "[b] highest concept accuracy overall. [a] anomalous at_junction collapse (see Section 5).",
    caption_style))
...
story += [
    ...
    p(
        "Holding architecture fixed at concept_ac, online supervision, frozen concept net, "
        "temporal encoding has the largest single effect in the experiment:"
    ),
]
q1_data = [
...
story += [
    p(
        ...
        "worst. The GRU hidden state is the mechanism that makes T-Maze tractable -- "
...
story += [
    ...
    p("At the best comparable settings (GRU, online supervision, frozen):"),
]
q2_data = [
...
q4_data = [
    ...
    ["none",   " 0.852", "0.750", "0.875  (best)"],
]
story.append(make_table(q4_data, [1.2*inch, 1.0*inch, 1.0*inch, 1.6*inch]))
story.append(p(
    "Table 6. Supervision mode ablation (concept_ac, GRU, frozen). "
    "'best' = highest concept accuracy in the entire experiment.",
    caption_style))
story += [
    p(
...
story += [
    ...
    p(
        ...
        "(1.000 in every other run). The GRU learned to encode the cue but the concept "
...
story += [
    ...
    p("The experiment establishes a clear hierarchy of factors for T-Maze performance:"),
]
...
summary_data = [
    ...
    ["3", "Gradient coupling",
     ...
     "label supervision is absent. End-to-end gradients can destabilize concept representations."],
    ["4", "Label supervision",
