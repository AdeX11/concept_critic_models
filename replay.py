...
from ppo.gvf import GVFConceptNetwork
...
def render_frame_from_obs(obs, env=None) -> np.ndarray:
    # Prefer env.render() — full-resolution, most-recent frame.
    if env is not None:
        rendered = env.render()
    ...
    # Fallback: extract from the stacked observation.
    if isinstance(obs, dict):
        obs = obs["images"]
    arr = np.asarray(obs)
    if arr.ndim == 3 and arr.shape[0] >= 3:
        # Channel-first stacked obs [C, H, W]; take the last (most recent) RGB triple.
...
def predict_step(
    ...
    if c_t is not None:
        if policy.method == "concept_actor_critic" and hasattr(policy.concept_net, "decode_concept_vector"):
            ...
            # c_t is [B, n_concepts + num_gvf]; keep only the concept columns
...
def replay(args) -> None:
    ...
    _uses_gru = args.temporal_encoding == "gru" and args.method in (
        "concept_actor_critic", "gvf"
    )
    h_t = (
...
def main() -> None:
    ...
    parser.add_argument(
        "--gvf_pairing", type=int, nargs="*", default=[],
        help="Concept indices paired with GVF heads (required when method=gvf). "
             "Must match the gvf_pairing used during training, e.g. --gvf_pairing 0 1",
    )
