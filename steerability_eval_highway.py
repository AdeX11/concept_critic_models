"""
steerability_eval_highway.py — Concept intervention experiment for Highway.
...
(separate episodes, fresh env each time):
  ...
  correct   — all 9 concepts forced to ground truth at every step
  flipped   — temporal concepts manipulated:
              • ego_vx (regression) → negated
              • blindspot_left_present (classification) → 0↔1 flipped
              • blindspot_left_rel_vx (regression) → negated
              • blindspot_right_present (classification) → 0↔1 flipped
              • blindspot_right_rel_vx (regression) → negated
...
Metrics per model:
  ...
  reward_flipped       mean reward if blindspot memory concepts were corrupted
...
Usage:
  python steerability_eval_highway.py
  python steerability_eval_highway.py --n_episodes 100 --results_dir /path/to/results
"""
...
from envs.highway import make_single_highway_state_env
...
MODELS = [
    ...
    {
        "tag":        "cbm_gru_online_frozen_highway_state_seed42",
        "concept_net":"cbm",
        "temporal":   "gru",
        "label":      "CBM | GRU | online | frozen",
    },
    {
        "tag":        "cbm_gru_online_coupled_highway_state_seed42",
        "concept_net":"cbm",
        "temporal":   "gru",
        "label":      "CBM | GRU | online | coupled",
    },
    {
        "tag":        "cbm_none_online_frozen_highway_state_seed42",
        "concept_net":"cbm",
        "temporal":   "none",
        "label":      "CBM | No memory | online | frozen",
    },
    {
        "tag":        "cbm_none_online_coupled_highway_state_seed42",
        "concept_net":"cbm",
        "temporal":   "none",
    ...
    {
        "tag":        "concept_ac_gru_online_frozen_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "gru",
        "label":      "Concept-AC | GRU | online | frozen",
    },
    {
        "tag":        "concept_ac_gru_online_coupled_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "gru",
        "label":      "Concept-AC | GRU | online | coupled",
    },
    {
        "tag":        "concept_ac_gru_none_frozen_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "gru",
        "label":      "Concept-AC | GRU | none | frozen",
    },
    {
        "tag":        "concept_ac_gru_none_coupled_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "gru",
        "label":      "Concept-AC | GRU | none | coupled",
    },
    {
        "tag":        "concept_ac_none_online_frozen_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "none",
        "label":      "Concept-AC | No memory | online | frozen",
    },
    {
        "tag":        "concept_ac_none_online_coupled_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "none",
        "label":      "Concept-AC | No memory | online | coupled",
    },
    {
        "tag":        "concept_ac_none_none_frozen_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "none",
        "label":      "Concept-AC | No memory | none | frozen",
    },
    {
        "tag":        "concept_ac_none_none_coupled_highway_state_seed42",
        "concept_net":"concept_ac",
        "temporal":   "none",
...
# Highway temporal concepts (indices 2, 5, 6, 7, 8):
#   2: ego_vx (regression)
#   5: blindspot_left_present (classification)
#   6: blindspot_left_rel_vx (regression)
#   7: blindspot_right_present (classification)
#   8: blindspot_right_rel_vx (regression)
TEMPORAL_INDICES = [2, 5, 6, 7, 8]

# Which temporal concepts are classification (binary 0/1)
CLASSIFICATION_TEMPORAL = {5, 7}
...
def load_policy(model_dir: str, concept_net: str, temporal: str,
    ...
    env = make_single_highway_state_env(seed=0)
    # Flat Box(25,) kinematics observation
...
def build_correct_override(true_concept: np.ndarray, n_concepts: int,
    ...
    """All 9 concepts set to ground truth."""
...
def build_flipped_override(true_concept: np.ndarray, n_concepts: int,
                           device: torch.device) -> torch.Tensor:
    """All concepts = ground truth, but temporal concepts are manipulated:
    - ego_vx (regression) → negated
    - blindspot_left_present (classification) → 0↔1 flipped
    - blindspot_left_rel_vx (regression) → negated
    - blindspot_right_present (classification) → 0↔1 flipped
    ...
    for idx in TEMPORAL_INDICES:
        if idx in CLASSIFICATION_TEMPORAL:
            # Classification: flip 0↔1
            c[idx] = 1.0 - c[idx]
        else:
            # Regression: negate
            c[idx] = -c[idx]
...
def evaluate_steerability(policy: ActorCriticPolicy, concept_net: str,
    ...
    """
    # ---- baseline ----
    env = make_single_highway_state_env(seed=seed)
    baseline_rewards = []
    baseline_lengths = []
    ...
    ceiling = max_observed + 1.0  # use observed ceiling since highway has no fixed max
...
def main():
    ...
    parser.add_argument("--results_dir", default="/glade/derecho/scratch/adadelek/results/highway_state")
    ...
    print(f"\nSteerability Evaluation — Highway")
    print(f"Episodes per model: {args.n_episodes}  |  seed: {args.seed}")
    print(f"Intervention targets: temporal concepts ego_vx (idx 2), blindspot_left_present (idx 5),")
    print(f"  blindspot_left_rel_vx (idx 6), blindspot_right_present (idx 7), blindspot_right_rel_vx (idx 8)")
    print(f"  Regression concepts negated, classification concepts flipped 0↔1")
    print("=" * 80)
